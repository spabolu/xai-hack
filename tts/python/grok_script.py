...
THE TEAM:
- Play-by-Play: Describes the action clearly and quickly.
- Color Commentator: Adds brief analysis or reaction.

NATURAL FLOW:
- They should build off each other's energy.
- React to each other naturally ("Absolutely!", "Good point.").

INSTRUCTIONS:
1. **NO CLICHES**: Do NOT use "Wow", "Oh my gosh", "Unbelievable", "Listen to this", or "Look at that".
2. **BE UNIQUE**: Use varied vocabulary. Instead of "Wow, what a dunk!", say "He hammers it home!" or "Rim-rattling jam!".
3. **CONCISE**: Max 1-2 sentences. This is for real-time TTS.
4. **NATURAL**: Speak exactly as a human broadcaster would. No "Script:" prefixes.

IMPORTANT:
- If multiple events are described, connect them fluently ("Steal by Curry, he pushes up court... and finds Thompson for three!").
- Do NOT output the commentator's name in the text.
"""
        # System prompt
#         # System prompt
...
# WHO SPEAKS:
# - Let the moment dictate who speaks.
# - Switch when it feels natural.

# Guidelines:
# - Write the commentary as a script - just the spoken words.
# - Be AUTHENTIC and PROFESSIONAL. 
# - Do NOT use phrases like "Oh my gosh", "Wow", or "Unbelievable" unless it is a buzzer-beater or a massive dunk.
# - Avoid repetitive hype. Focus on the technical basketball action and narrative.
...
async def process_events_pipelined(
    ...
    """
    Smart Pipeline with Event Merging:
    - Task A (Brain): Buffers skipped events and merges them into the next prompt.
    - Task B (Voice): Uses dynamic excitement levels to prevent excessive screaming.
    """
    import asyncio
    from tts import text_to_speech_excited
    import subprocess
    ...
    # 1. Shared Queue
    # Items: (text, commentator, event_id, excitement_level)
    ...
    print(
        f"🚀 Starting Smart Pipeline (Speed: {speed_multiplier}x)",
        file=sys.stderr,
    )
    ...
    async def voice_worker():
        ...
        while True:
            ...
            # Clean text (remove names)
            clean_text = re.sub(
                r"^(?:Ara|Eve|Leo|Rex|Sal|Una|Commentator|Announcer):\s*",
                "",
                text,
                flags=re.IGNORECASE,
            )
            ...
            try:
                q_size = script_queue.qsize()
                print(
                    f"🎵 Generating audio [{q_size} queued] ({excitement})...",
                    file=sys.stderr,
                )

                # Use the ACTUAL excitement level from Grok, not hardcoded "high"
                # This prevents the TTS from forcing "Oh my gosh" if the model adds it
    ...
    # Context Buffer for skipped events
    buffered_descriptions = []

    CRITICAL_KEYWORDS = ["made", "dunk", "3pt", "foul", "timeout", "end"]

    for i, event in enumerate(events):
        event_time = event.get("timeActual", 0)
        description = event.get("description", "")

        # 1. Simulate Real-Time Pacing
        time_diff = event_time - last_event_time
        if time_diff > 0 and i > 0:
            await asyncio.sleep(time_diff / speed_multiplier)
        last_event_time = event_time

        # 2. Backlog Check & Merging Logic
        backlog_size = script_queue.qsize()
        is_critical = any(k in description.lower() for k in CRITICAL_KEYWORDS)

        # If we are lagging and this is NOT a critical play, BUFFER IT
        if backlog_size >= 2 and not is_critical:
            print(
                f"⏩ [Lag: {backlog_size}] Buffering: {description[:30]}...",
                file=sys.stderr,
            )
            buffered_descriptions.append(description)
            continue  # Skip processing this specific cycle, move to next event

        # 3. Prepare Event Data
        # If we have buffered events, merge them into this one
        event_to_process = event.copy()
        if buffered_descriptions:
            context_str = "; ".join(buffered_descriptions)
            print(
                f"🔗 Merging {len(buffered_descriptions)} previous events...",
                file=sys.stderr,
            )

            # Rewrite description to include history
            event_to_process["description"] = (
                f"Previous action: {context_str}. CURRENT PLAY: {description}"
            )
            # Clear buffer
            buffered_descriptions = []
        ...
        try:
            ...
            commentary = await asyncio.wait_for(
                asyncio.to_thread(agent.process_event, event_to_process), timeout=15.0
            )
            ...
            await script_queue.put(
                (
                    ...
                    i,
                    commentary.excitement_level,  # Pass dynamic excitement
                )
            )

        except asyncio.TimeoutError:
            print(f"❌ Grok timed out (>15s), buffering event...", file=sys.stderr)
            # If timed out, save this description to try again with the next event
            buffered_descriptions.append(description)
        except Exception as e:
